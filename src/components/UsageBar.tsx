...
export default function UsageBar() {
  ...
  return (
    <>
      ...
      >
        <div className="w-24 h-1 bg-[var(--color-border)] overflow-hidden">
          <div
            className={`h-full transition-all ${usage.throttled ? "bg-orange-600" : pct > 80 ? "bg-[var(--color-brand)]" : "bg-[var(--color-fg)]"}`}
            style={{ width: `${pct}%` }}
          />
        </div>
        <span className="text-[var(--color-fg-muted)] font-mono">{usage.used}/{usage.limit}</span>
        {atLimit && (
          <span className="text-[var(--color-brand)] uppercase tracking-[0.15em] hover:text-[var(--color-fg)]">Upgrade</span>
        )}
      </div>
    </>
  );
