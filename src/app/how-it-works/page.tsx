  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="max-w-4xl mx-auto px-6 py-16">
        <div className="grid md:grid-cols-3 gap-6">
          {steps.map((step) => (
            ...
            >
              <div className="w-8 h-8 rounded-[2px] bg-[var(--color-surface)] text-[var(--color-brand)] border border-[var(--color-border)] flex items-center justify-center text-sm font-bold mb-4">
                {step.num}
              </div>
      ...
      <section className="max-w-3xl mx-auto px-6 py-16">
        ...
        <p className="text-[var(--color-fg-muted)] text-center mb-10 max-w-xl mx-auto">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {[
            ...
            <div key={dim.label} className="bg-[var(--color-surface-raised)] border border-[var(--color-border)] rounded-[2px] p-4">
              <p className="font-medium text-sm text-[var(--color-brand)] mb-1">{dim.label}</p>
              <p className="text-[var(--color-fg-muted)] text-sm leading-relaxed">{dim.detail}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-3xl mx-auto px-6 py-16">
        ...
        <div className="space-y-4">
          {faqs.map((faq) => (
            <details key={faq.q} className="bg-[var(--color-surface-raised)] border border-[var(--color-border)] rounded-[2px] group">
              <summary className="px-6 py-4 cursor-pointer text-sm font-medium list-none flex items-center justify-between">
                {faq.q}
                <span className="text-[var(--color-fg-muted)] group-open:rotate-45 transition-transform text-lg">+</span>
              </summary>
              <div className="px-6 pb-4 text-sm text-[var(--color-fg-muted)] leading-relaxed">
                {faq.a}
              </div>
            </details>
          ))}
      ...
      <section className="max-w-4xl mx-auto px-6 py-12">
        <div className="grid sm:grid-cols-4 gap-4">
          <Link
            href="/write"
            className="bg-[var(--color-surface-raised)] border border-[var(--color-border)] rounded-[2px] p-4 transition-colors hover:border-[var(--color-fg)] block text-center"
          >
            <p className="font-medium text-sm mb-1">Use Cases</p>
            <p className="text-xs text-[var(--color-fg-muted)]">Speeches, emails, essays & more</p>
          </Link>
          <Link
            href="/analyze"
            className="bg-[var(--color-surface-raised)] border border-[var(--color-border)] rounded-[2px] p-4 transition-colors hover:border-[var(--color-fg)] block text-center"
          >
            ...
            <p className="text-xs text-[var(--color-fg-muted)]">See your writing style mapped out</p>
          </Link>
          <Link
            href="/write-like"
            className="bg-[var(--color-surface-raised)] border border-[var(--color-border)] rounded-[2px] p-4 transition-colors hover:border-[var(--color-fg)] block text-center"
          >
            ...
            <p className="text-xs text-[var(--color-fg-muted)]">Write like Hemingway, Didion & more</p>
          </Link>
          <Link
            href="/pricing"
            className="bg-[var(--color-surface-raised)] border border-[var(--color-border)] rounded-[2px] p-4 transition-colors hover:border-[var(--color-fg)] block text-center"
          >
            ...
            <p className="text-xs text-[var(--color-fg-muted)]">Free forever, Pro for $19/mo</p>
          </Link>
        </div>
      </section>
      ...
      <footer className="border-t border-[var(--color-border)] py-8">
        <div className="max-w-6xl mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-x-6 gap-y-2">
            ...
            <Link href="/blog" className="text-xs text-[var(--color-fg-muted)] hover:text-[var(--color-fg)] transition-colors">Blog</Link>
